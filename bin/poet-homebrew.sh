...
URL=$(perl -n000e 'print $1 while /^..resource\s\"trump-net\"\sdo\n\s+url\s\"(.*?)\"\n.*?\n..end\n\n/mg' /tmp/poet.rb)

# remove package resource from poet manifest
perl -0777 -i.original -pe 's/..resource\s\"trump-net\"\sdo\n.*?\n.*?\n..end\n\n//igs' /tmp/poet.rb
...
cat > /tmp/pkg.rb <<-EOF
# Homebrew Formula
# trump-net, Ian Dennis Miller
# rebuild with 'make homebrew'

class trump_net < Formula
  desc "Model the Trump Situation"
  homepage "https://github.com/iandennismiller/trump-net"
  url "${URL}"
  sha256 "${SHA}"
